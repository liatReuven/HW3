...
export default function Register(props) {
  ...
  const [updateUser, setFormData] = useState({
    username: '',
    email: '',
    password: '',
    dob: '',
  ...
  useEffect(() => {
    const loggedInUser = JSON.parse(sessionStorage.getItem('currentUser')); //bring the user that is now looged in
  ...
  const handleChange = (e) => {
    ...
    if (name === 'confirmPassword' && value !== updateUser.password) {
  ...
  const EditUser = (e) => {
    ...
    let updatedUser = {
      ...
      confirmPassword: updateUser.confirmPassword || user.confirmPassword,
      selectedCity: updateUser.selectedCity || user.selectedCity,
      street: updateUser.street || user.street,
      number: updateUser.numberInput || user.numberInput,
      firstName: updateUser.firstName || user.firstName,
      lastName: updateUser.lastName || user.lastName,
      dob: updateUser.dob || user.dob,
      file: updateUser.file || user.file,
      email: user.email, //email can not be changed
    };

    console.log('Form Data Submitted:', updatedUser);

    props.SendToParent(updatedUser); //send the new user to the parent(App)
    navigate('/profile')
  };
  ...
  return (
    <div className='page-register'>
      <form className='Register-form'>
        <h2> <EditIcon style={{ width: '45px' }} />Edit Details</h2>
        ...
        <FormControl sx={{ display: 'flex', gap: '10px' }}>
          <Stack direction="row" gap={'10px'} className="form-input" >
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label="Select City" variant="outlined" value={user?.selectedCity || ''} />
              )}
            />
          </Stack>
        </FormControl>
